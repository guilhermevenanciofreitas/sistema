...
export default class FreightCalculationsBase extends BaseIndex {
    ...
    componentDidMount = async () =>
    {
        try
        {
            ...
            if (id) {
                await this.FreightCalculation(id.toString(), false);
            }
    ...
    protected BtnEdit_Click = async (id: string): Promise<void> =>
    {
        try
        {
            ...
            const r = await this.FreightCalculation(id);
    ...
    protected BtnNovo_Click = async (): Promise<void> =>
    {
        try
        {
            ...
            const r = await this.ViewFreightCalculation.current?.Show(undefined);
    ...
    protected CardStatus_Click = async(typeId?: string | null): Promise<void> =>
    {
        try
        {
            this.setState({request: {...this.state.request, typeId}}, 
                async () => await this.Pesquisar(this.state.request)
            );
        }
        catch (err: any) 
    ...
    protected BtnPesquisar_Click = async(): Promise<void> =>
    {
        try
        {
            this.setState({request: {...this.state.request, typeId: undefined}}, 
                async () => await this.Pesquisar(this.state.request)
            );
        }
        catch (err: any) 
    ...
    private FreightCalculation = async (id: string, isHitoryBack: boolean = true) =>
    {
        history.pushState(null, "", `${window.location.origin}${window.location.pathname}?id=${id}`);
        const r = await this.ViewFreightCalculation.current?.Show(id);
