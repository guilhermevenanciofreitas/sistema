            ...
            <ViewModal ref={this.ViewModal} Title={this.props.Title} Width={1000}>
                <Content>
                    ...
                    <Grid container spacing={1} sx={{ flexGrow: 1 }}>
                        ...
                        <Grid md={4}>
                            <AutoComplete Label='Fornecedor' Action={{Type: 'Supplier', New: {Values: {}}, Edit: {Id: _.get(this.state.supplier, 'id')}}} Pesquisa={async(Text: string) => await Search.Supplier(Text)} Text={(Item: any) => `${Item.surname}` } Value={this.state.supplier} OnChange={async (supplier: any) => {
                                if (supplier?.isBlockBuy) {
                        ...
                        <Grid md={4}>
                            <AutoComplete Label='Comprador' Action={{Type: 'Employee', New: {Values: {}}, Edit: {Id: _.get(this.state.buyer, 'id')}}} Pesquisa={async(Text: string) => await Search.Employee(Text)} Text={(Item: any) => `${Item.surname}` } Value={this.state.buyer} OnChange={(buyer: any) => this.setState({buyer})}>
                                <EmployeeTemplate />
                            </AutoComplete>
                        </Grid>
                        <Grid md={2}>
